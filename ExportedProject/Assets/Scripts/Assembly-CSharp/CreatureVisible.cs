...
public class CreatureVisible : MonoBehaviour
{
	...
	public static bool IsVisibleUnit<T>(T unit, Transform from, float angle, float distance, LayerMask mask) where T : CreatureVisible
	{
		bool result = false;
		if (unit != null)
		{
			...
			{
				if (IsVisibleObject(from, transform.position, unit.gameObject, angle, distance, mask))
				{
					result = true;
					break;
				}
	...
	public bool IsCreatureVisibleFor(CreatureEyes eyes)
	{
		...
		return IsVisibleUnit(this, eyes.GetEyesTransfrom(), eyes.horAngle / 2f, eyes.distance, eyes.layers);
	}
